...
const Onboarding: React.FC<Props> = ({ onComplete, onCancel }) => {
  ...
  if (safetyBlocked) {
    return (
      <div className="max-w-xl mx-auto mt-10 p-8 bg-white rounded-[2.5rem] shadow-xl shadow-emerald-900/5 border border-emerald-50 animate-fade-in text-center">
        ...
        <p className="text-slate-600 mb-8 leading-relaxed">
          Para que tu camino hacia una vida más saludable sea 100% seguro y efectivo, en tu caso particular te recomendamos <strong>consultar con un profesional sanitario</strong> antes de iniciar cualquier plan automatizado. ¡Tu salud es lo más importante!
        </p>
        <button 
          onClick={onCancel}
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 md:py-10">
      ...
      <div className="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-50 min-h-[500px] flex flex-col">
        ...
        {currentStep === 2 && (
          <div className="animate-fade-in space-y-8 flex-grow">
            <header>
              ...
              <p className="text-slate-500 mt-2">Estos datos nos ayudan a calcular tu energía diaria ideal.</p>
            </header>
        ...
        {currentStep === 3 && (
          <div className="animate-fade-in space-y-8 flex-grow">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-3">Nivel de Actividad Semanal</label>
                <div className="grid grid-cols-2 gap-3">
                  <ActivityBtn active={data.activity === ActivityLevel.SEDENTARY} label="Suave" desc="Día a día tranquilo" onClick={() => setData({...data, activity: ActivityLevel.SEDENTARY})} />
                  <ActivityBtn active={data.activity === ActivityLevel.LIGHT} label="Activo" desc="1-2 veces deporte" onClick={() => setData({...data, activity: ActivityLevel.LIGHT})} />
                  <ActivityBtn active={data.activity === ActivityLevel.MODERATE} label="Enérgico" desc="3-4 veces deporte" onClick={() => setData({...data, activity: ActivityLevel.MODERATE})} />
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-3">Tu Preferencia</label>
        ...
        {currentStep === 4 && (
          <div className="animate-fade-in space-y-8 flex-grow">
            <header>
              ...
              <p className="text-slate-500 mt-2">Personaliza cómo y cuándo quieres disfrutar de tus platos.</p>
            </header>
            <div className="space-y-4">
              <label className="block text-sm font-bold text-slate-700 mb-2">Modalidad de Alimentación</label>
              <div className="grid gap-3">
                <FastingOption 
                  active={data.fastingType === 'none'} 
                  onClick={() => setData({...data, fastingType: 'none'})}
                  title="Horario Tradicional"
                  desc="Sin restricciones. Distribución constante todo el día."
                  icon="⏰"
                />
                <FastingOption 
                  active={data.fastingType === '12-20'} 
                  onClick={() => setData({...data, fastingType: '12-20'})}
                  title="Ayuno Social (12h - 20h)"
                  desc="Perfecto si prefieres saltarte el desayuno y cenar tarde."
                  icon="🌙"
                />
                <FastingOption 
                  active={data.fastingType === '9-17'} 
                  onClick={() => setData({...data, fastingType: '9-17'})}
                  title="Ayuno Madrugador (09h - 17h)"
                  desc="Ideal para empezar el día con energía y cenar ligero."
                  icon="☀️"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2 tracking-widest text-center">Presupuesto</label>
                  <select value={data.budget} onChange={e => setData({...data, budget: e.target.value as any})} className="w-full p-4 bg-slate-50 border-0 rounded-2xl font-bold text-slate-700">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2 tracking-widest text-center">Cocina</label>
                  <select value={data.cookingTime} onChange={e => setData({...data, cookingTime: e.target.value as any})} className="w-full p-4 bg-slate-50 border-0 rounded-2xl font-bold text-slate-700">
                    <option value="rapido">Exprés</option>
                    <option value="normal">Con tiempo</option>
                  </select>
                </div>
              </div>
            </div>
        ...
        <div className="mt-auto flex gap-4 pt-8 border-t border-slate-50">
          <button onClick={prev} className="px-6 md:px-8 py-4 bg-slate-100 text-slate-600 font-bold rounded-2xl hover:bg-slate-200 transition-all flex items-center gap-2">
            <svg className="w-4 h-4 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
            <span className="hidden sm:inline">Atrás</span>
          </button>
          <button 
            onClick={currentStep === totalSteps ? () => onComplete(data) : next}
            className="flex-grow px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl shadow-lg shadow-emerald-100 transition-all transform active:scale-95 flex items-center justify-center gap-2"
          >
...
const FastingOption = ({ active, onClick, title, desc, icon }: any) => (
  <button 
    onClick={onClick}
    className={`w-full p-4 rounded-2xl border-2 text-left transition-all flex items-center gap-4 ${active ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 bg-slate-50 hover:bg-slate-100'}`}
  >
    <div className="text-2xl shrink-0">{icon}</div>
    <div className="flex-grow">
      <p className={`font-bold block ${active ? 'text-emerald-700' : 'text-slate-700'}`}>{title}</p>
      <p className="text-[11px] text-slate-400 font-medium leading-tight">{desc}</p>
    </div>
    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${active ? 'border-emerald-600 bg-emerald-600' : 'border-slate-200'}`}>
...
const SafetyCard = ({ label, onCheck }: any) => (
  <label className="flex items-center gap-4 p-4 border border-slate-100 rounded-2xl hover:bg-emerald-50 hover:border-emerald-100 cursor-pointer transition-all group">
    <input type="checkbox" onChange={e => e.target.checked && onCheck()} className="w-5 h-5 rounded border-slate-200 text-emerald-500 focus:ring-emerald-500" />
    <span className="text-slate-700 font-medium group-hover:text-emerald-800">{label}</span>
  </label>
);

const ToggleButton = ({ active, onClick, label }: any) => (
  <button onClick={onClick} className={`flex-1 py-4 rounded-2xl font-bold transition-all ${active ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}>
    {label}
  </button>
);
...
const ActivityBtn = ({ active, label, desc, onClick }: any) => (
  <button onClick={onClick} className={`p-4 rounded-2xl border-2 text-left transition-all ${active ? 'border-emerald-600 bg-emerald-50' : 'border-slate-50 bg-slate-50 opacity-60 hover:opacity-100'}`}>
    <p className={`font-bold block ${active ? 'text-emerald-700' : 'text-slate-700'}`}>{label}</p>
