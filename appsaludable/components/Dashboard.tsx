...
const Dashboard: React.FC<Props> = ({ plan, userData, nutrition, onFinishWeek }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-fade-in">
      ...
      <div className="bg-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 no-print">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          >
            ...
            <span className="text-slate-600 group-hover:text-emerald-700 font-black text-sm uppercase tracking-tight">Imprimir</span>
          </button>
          <button 
  ...
  <div className={`bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-50 overflow-hidden ${isPrintMode ? 'shadow-none border-slate-100' : ''}`}>
    ...
    <div className="p-6 md:p-12 space-y-8 md:space-y-12">
      {day.meals.map((meal: any, mIdx: number) => (
        <div key={mIdx} className="meal-section border-b-2 border-slate-50 last:border-0 pb-8 md:pb-12 last:pb-0">
          <div className="flex justify-between items-start gap-4 mb-6 md:mb-8">
            <div>
              <div className="flex items-center gap-3 mb-2 md:mb-3">
                ...
                <span className="text-slate-500 font-black text-[10px] md:text-xs uppercase">{meal.time}</span>
              </div>
              <h3 className="text-xl md:text-2xl font-black text-slate-800 leading-tight">{meal.name}</h3>
            </div>
            <span className="text-slate-500 font-black text-xs md:text-sm whitespace-nowrap">{meal.calories} kcal</span>
          </div>

          <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            <div className="bg-slate-50 p-5 md:p-6 rounded-2xl border border-slate-100">
              <h4 className="text-[10px] md:text-[11px] font-black text-slate-500 uppercase tracking-widest mb-4 md:mb-5 border-b border-slate-200 pb-2">Ingredientes</h4>
              <ul className="text-xs md:text-sm text-slate-600 space-y-2 md:space-y-3 font-semibold">
            ...
            <div className="md:col-span-2">
              <h4 className="text-[10px] md:text-[11px] font-black text-slate-500 uppercase tracking-widest mb-4 md:mb-5 border-b border-slate-50 pb-2">Instrucciones de Cocina</h4>
              <div className="space-y-3 md:space-y-4">
                {meal.instructions.map((ins: string, i: number) => (
                  <div key={i} className="flex gap-4 md:gap-5">
                    <span className="bg-emerald-50 text-emerald-700 w-6 h-6 md:w-7 md:h-7 rounded-full flex items-center justify-center font-black text-[10px] md:text-[11px] shrink-0 border border-emerald-100">{i+1}</span>
                    <p className="text-xs md:text-sm text-slate-600 leading-relaxed font-semibold">{ins}</p>
                  </div>
                ))}
              </div>
            </div>
...
const StatCard: React.FC<any> = ({ label, value, sub, color }) => (
  <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
    <p className="text-[9px] md:text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 md:mb-3">{label}</p>
    <div>
      <h4 className={`text-xl md:text-2xl font-black text-${color}-600 leading-tight`}>{value}</h4>
      <p className="text-[9px] md:text-[10px] text-slate-400 font-black italic mt-0.5 uppercase tracking-wider">{sub}</p>
    </div>
  </div>
);
