      ...
      <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 no-print">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button 
            ...
            className="flex-1 md:flex-none bg-emerald-600 text-white px-6 py-3.5 rounded-2xl font-black text-sm md:text-base shadow-lg shadow-emerald-100 transition-all hover:bg-emerald-700 transform active:scale-95 uppercase tracking-tight"
          >
            Nuevo Plan
          </button>
        </div>
  ...
  <div className={`bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border-2 border-slate-100 overflow-hidden ${isPrintMode ? 'shadow-none border-slate-200' : ''}`}>
    <div className="bg-emerald-600 p-6 md:p-8 text-white flex justify-between items-center">
      ...
      <div className="text-right">
        <span className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-xl text-sm md:text-xl font-black border border-white/30 inline-block">{day.totalCalories} kcal</span>
    ...
    <div className="p-6 md:p-10 space-y-10">
      {day.meals.map((meal: any, mIdx: number) => (
        <div key={mIdx} className="meal-section border-b-2 border-slate-50 last:border-0 pb-10 last:pb-0">
